...
static bool attachUniqueInstance(const QStringList &paths, const QString &serializedOptions)
{
    ...
    QDBusInterface iface(QStringLiteral("io.github.todd866.paperlibrary"), QStringLiteral("/paperlibraryshell"), QStringLiteral("io.github.todd866.paperlibrary"));
    if (!iface.isValid()) {
        return false;
    }
    ...
    if (!ShellUtils::editorCmd(serializedOptions).isEmpty()) {
        QString message =
            i18n("You cannot set the editor command in an already running PaperLibrary instance. Please disable the tabs and try again. Please note, that unique is also not supported when setting the editor command at the commandline.\n");
...
static bool attachExistingInstance(const QStringList &paths, const QString &serializedOptions)
{
    ...
    for (const QString &service : services) {
        if (service.startsWith(ShellUtils::kPerProcessDbusPrefix) && service != ownDbus) {
            bestService.reset(new QDBusInterface(service, QStringLiteral("/paperlibraryshell"), QStringLiteral("io.github.todd866.paperlibrary")));
...
Status main(const QStringList &paths, const QString &serializedOptions)
{
    ...
    // A launch without a file argument brings back the last session's
    // document tabs; an explicitly opened file suppresses the restore and
    // opens alone, matching Preview
    if (paths.isEmpty()) {
        shell->restoreSavedTabs();
    }
