...
#include "../shell/paperlibraryaboutdata.h"
...
private Q_SLOTS:
    ...
    void testUrlArgs();
    void testAboutDataBranding();
};

// The application identity is PaperLibrary so day-one installs write
// PaperLibrary config files directly.
void ShellTest::testAboutDataBranding()
{
    const KAboutData about = paperLibraryAboutData();

    QCOMPARE(about.displayName(), QStringLiteral("PaperLibrary"));
    QCOMPARE(about.componentName(), QStringLiteral("paperlibrary"));
    QVERIFY(about.homepage().contains(QStringLiteral("github.com/todd866")));

    // GPL attribution: the fork author is credited AND upstream Okular
    // authors are retained.
    bool foundForkAuthor = false;
    bool foundUpstreamAuthor = false;
    const QList<KAboutPerson> authors = about.authors();
    for (const KAboutPerson &person : authors) {
        if (person.name() == QStringLiteral("Ian Todd")) {
            foundForkAuthor = true;
        }
        if (person.name() == QStringLiteral("Albert Astals Cid")) {
            foundUpstreamAuthor = true;
        }
    }
    QVERIFY(foundForkAuthor);
    QVERIFY(foundUpstreamAuthor);
}
